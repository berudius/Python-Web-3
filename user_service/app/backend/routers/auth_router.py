...
async def get_all_users_list(db: Session = Depends(get_db)):
    ...
    # аналогічно до того, як  це зроблено в get_user
    users_data = [{
        "id": user.id,
        "login": user.login,
        "role": user.role,
...
async def update_user_details(
    ...
    #  КРИТИЧНО ВАЖЛИВО: exclude_unset=True
    # Цей параметр каже Pydantic'у: "Дай мені словник тільки з тих полів, 
    # які клієнт реально надіслав у JSON".
    # Без цього Pydantic перетворив би пропущені поля в None, і затерлися б дані в БД.
